...
* **Codenames** are generated automatically generated by Kentico Cloud based on the object's name. They can make your code more readable, but are not guaranteed to be unique. They should only be used for small projects with no chance of naming conflicts. 
* (internal) **IDs** are random Guids assingned to objects by Kentico Cloud at the momemt of import/creation. They are unique, but only objects that are already in the system have them. 
* **External IDs** are string-based custom identifiers defined by you. This is useful When importing a batch of cross-referencing content. 
